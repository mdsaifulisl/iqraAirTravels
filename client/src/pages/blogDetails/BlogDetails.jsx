...
const BlogDetails = () => {
  ...
  return (
    <div className="blog-details-page pb-5 overflow-hidden">
      {/* Hero Header */}
      <section
        className="blog-detail-hero py-5"
        style={{ backgroundColor: "var(--accent-alice-blue)" }}
      >
        <div className="container">
          <button
            onClick={() => navigate(-1)}
            className="btn btn-link text-teal fw-bold mb-4 p-0 text-decoration-none shadow-none"
          >
            <FaArrowLeft className="me-2" /> Back to Blog
          </button>
          ...
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb mb-4">
              ...
              <li
                className="breadcrumb-item active text-teal small"
                aria-current="page"
              >
                {post.category}
              </li>
          ...
          <div className="d-flex flex-wrap gap-4 text-muted small border-top pt-4">
            <span className="d-flex align-items-center gap-2">
              <FaUser className="text-coral" /> By {post.author}
            </span>
            <span className="d-flex align-items-center gap-2">
              <FaCalendarAlt className="text-coral" /> {post.date}
            </span>
            <span className="d-flex align-items-center gap-2">
              <FaClock className="text-coral" /> 5 Min Read
            </span>
            <span className="d-flex align-items-center gap-2">
              <FaTag className="text-coral" /> {post.category}
            </span>
          </div>
        </div>
      </section>
        ...
        <div className="row g-5">
          <div className="col-lg-8">
            ...
            {post.images && post.images.length > 0 && (
              <div
                className="blog-hero-container position-relative mb-5 rounded-4 overflow-hidden shadow-sm"
              ...
              >
                ...
                {post.images.length > 1 && (
                  <div
                    className="slider-dots d-flex gap-2 position-absolute bottom-0 start-50 translate-middle-x mb-3"
                    style={{ zIndex: 5 }}
                  >
                    {post.images.map((_, i) => (
                      <span
          ...
          <div className="col-lg-4">
            <div
              className="blog-sidebar position-sticky"
              style={{ top: "100px" }}
            >
              <div className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
                ...
                <div className="input-group">
                  <input
                    type="text"
                    className="form-control border-light bg-light shadow-none"
              ...
              <div className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
                ...
                <ul className="list-unstyled mb-0">
                  {[
                    { name: "Visa Tips", path: "/visa-service", count: 12 },
                    { name: "Travel Guide", path: "/tours", count: 12 },
                    { name: "Destinations", path: "/destinations", count: 12 },
                    { name: "Travel blogs", path: "/blog", count: 12 },
                  ].map((cat, i) => (
                    <li
                      key={i}
                      className="mb-2 border-bottom border-light pb-2"
                    >
                      <Link
                        to={cat.path}
                        className="text-decoration-none text-muted d-flex justify-content-between align-items-center hover-teal transition-all"
                      >
                        {cat.name}
                        <span className="badge rounded-pill bg-alice-blue text-teal fw-normal">
                          {cat.count}
                        </span>
                      </Link>
                    </li>
