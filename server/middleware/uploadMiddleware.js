...
// ২. ফাইল ফিল্টার (PDF ও কমন ছবি ফরম্যাট সাপোর্টসহ)
const fileFilter = (req, file, cb) => {
  const allowedFileTypes = /jpeg|jpg|png|webp|pdf|svg|ico/;
  const extname = allowedFileTypes.test(path.extname(file.originalname).toLowerCase());
  const mimetype = allowedFileTypes.test(file.mimetype);
  ...
  if (extname && mimetype) {
    ...
    cb(new Error('শুধুমাত্র ছবি (JPG, PNG, WEBP, SVG) এবং PDF ফাইল আপলোড করা যাবে!'), false);
  }
};
...
const upload = multer({
  storage: storage,
  // 🚨 [FIX 3] মোবাইলের অরিজিনাল ছবির জন্য ফাইল সাইজ ২৫ MB করা হলো
